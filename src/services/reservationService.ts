...
class ReservationService {
  ...
  async getReservationAttendees(
    ...
    try {
      ...
      if (filter.reservationStatus) params.append('status', filter.reservationStatus);
  ...
  async downloadAttendeesExcel(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`엑셀 다운로드 API 호출 실패: ${response.status} ${response.statusText}`);
      }
  ...
  transformToUIData(dto: ReservationAttendeeDto): ReservationUIData {
    ...
    return {
      ...
      // 상태명을 한글로 변환
      reservationStatus: RESERVATION_STATUS_NAMES[reservationStatus] || dto.reservationStatus
    };
  }
