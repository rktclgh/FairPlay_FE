...
export const hostPaymentAPI = {
  /**
   * 결제 목록 조회 (행사 담당자용)
   */
  async getPaymentList(criteria: HostPaymentSearchCriteria): Promise<ApiResponse<PaymentAdminDto>> {
    try {
   ...
   * 결제 통계 정보 조회 (행사 담당자용)
   */
  async getPaymentStatistics(criteria: HostPaymentSearchCriteria): Promise<PaymentStatistics> {
    try {
      const params = buildQueryParams(criteria);
      const response = await authManager.authenticatedFetch(
   ...
   * 엑셀 다운로드 (행사 담당자용)
   */
  async exportPaymentExcel(criteria: HostPaymentSearchCriteria): Promise<Blob> {
    try {
      const params = buildQueryParams(criteria);
      const response = await authManager.authenticatedFetch(
