...
import { paymentAdminAPI } from '../services/paymentAdmin.service';

const initialSearchCriteria: PaymentSearchCriteria = {
  paymentTypes: [],
  paymentStatuses: [],
  startDate: undefined,
  endDate: undefined,
...
export const usePaymentManagement = () => {
  ...
  // 검색 조건 업데이트
  const setSearchCriteria = useCallback((criteria: PaymentSearchCriteria) => {
    setState(prev => ({
      ...prev,
  ...
  // 결제 목록 조회
  const fetchPayments = useCallback(async (criteria?: PaymentSearchCriteria) => {
    const searchParams = criteria || state.searchCriteria;
    
    setState(prev => ({ ...prev, loading: true, error: null }));

    try {
      const response: ApiResponse<PaymentAdminDto> = await paymentAdminAPI.getPaymentList(searchParams);
  ...
  // 결제 통계 조회
  const fetchStatistics = useCallback(async (criteria?: PaymentSearchCriteria) => {
    const searchParams = criteria || state.searchCriteria;

    try {
      const statistics: PaymentStatistics = await paymentAdminAPI.getPaymentStatistics(searchParams);
      setState(prev => ({
        ...prev,
        statistics
      }));
  ...
  const handleExportExcel = useCallback(async () => {
    try {
      ...
      const blob = await paymentAdminAPI.exportPaymentExcel(state.searchCriteria);
      ...
      link.download = `payment_list_${timestamp}.xlsx`;
