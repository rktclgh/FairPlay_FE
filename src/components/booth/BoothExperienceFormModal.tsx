...
import { X, Calendar, Clock, Users, FileText, Settings, Save } from 'lucide-react';
import {
...
const BoothExperienceFormModal: React.FC<BoothExperienceFormModalProps> = ({
  ...
  useEffect(() => {
    const loadBooths = async () => {
      try {
        ...
        // 신규 등록이고 부스가 1개만 있으면 자동 선택
        if (!isEditMode && boothList.length === 1) {
  ...
  const handleInputChange = (field: keyof BoothExperienceRequest, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              {!isEditMode && (
                <div>
                  <label htmlFor="boothId" className="block text-sm font-medium text-gray-700 mb-1">
                    부스 선택 <span className="text-red-500">*</span>
                  </label>
                  <select
                    id="boothId"
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                      errors.boothId ? 'border-red-300' : 'border-gray-300'
                    }`}
                    value={selectedBoothId || ''}
                    onChange={(e) => setSelectedBoothId(e.target.value ? parseInt(e.target.value) : null)}
                  >
                    <option value="">부스를 선택하세요</option>
                    {booths.map((booth) => (
                      <option key={booth.boothId} value={booth.boothId}>
                        {booth.boothTitle} ({booth.eventTitle})
                      </option>
                    ))}
                  </select>
                  {errors.boothId && <p className="text-red-500 text-sm mt-1">{errors.boothId}</p>}
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  id="experienceDate"
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                    errors.experienceDate ? 'border-red-300' : 'border-gray-300'
                  }`}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    max="200"
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                      errors.maxWaitingCount ? 'border-red-300' : 'border-gray-300'
                    }`}
                    placeholder="10"
